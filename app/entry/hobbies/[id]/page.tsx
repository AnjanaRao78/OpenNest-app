...
export default function HobbiesDetailPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      const data = await loadEntryById("hobbies", id);
      setEntry(data);
      setLoading(false);
    }
  ...
  return (
    <div className="max-w-2xl mx-auto p-6">
      <div className="rounded-2xl border p-6 bg-white shadow">
        <h1 className="text-2xl font-bold mb-4">{entry.hobbyName}</h1>

        <div className="space-y-2 text-sm">
          <p><strong>Author:</strong> {entry.authorName}</p>
          <p><strong>Frequency Goal:</strong> {entry.frequencyGoal}</p>
